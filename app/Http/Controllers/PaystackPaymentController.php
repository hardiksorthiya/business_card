        ...
        if($invoice)
        {
            try
            {
                ...
                if(isset($result['status']) && $result['status'] == true)
                {
                    ...
                    return redirect()->route('customer.invoice.show', Crypt::encrypt($invoice->id))->with('success', __(' Payment successfully added.'));

                }
                else
                {
                    return redirect()->route('customer.invoice.show', Crypt::encrypt($invoice->id))->with('error', __('Transaction Unsuccesfull'));
                }
            }
            catch(\Exception $e)
            {
                return redirect()->route('customer.invoice.show', Crypt::encrypt($invoice->id))->with('error', __('Transaction has been failed.'));
            }
        }
        else
        {
            return redirect()->route('customer.invoice.show', Crypt::encrypt($invoice->id))->with('error', __('Invoice is deleted.'));
        }
    }
}
