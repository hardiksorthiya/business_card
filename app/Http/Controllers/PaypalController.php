    ...
    public function cardPayWithPaypal(Request $request,$id)
    {
        ...
        $provider = new PayPalClient;
            ...
            if (isset($response['id']) && $response['id'] != null) {
                ...
                return redirect(url('/' . $business->slug))->with('error', $response['message'] ?? 'Something went wrong.');
            }
    ...
    public function cardGetPaymentStatus(Request $request, $business_id)
    {
        ...
        $provider = new PayPalClient;
            ...
            if (isset($response['status']) && $response['status'] == 'COMPLETED') {
                ...
                return redirect(url('/' . $business->slug))->with('error', 'Transaction has been failed..');
            }
